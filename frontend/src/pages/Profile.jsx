...
{postType=="posts" && postData.map((post,index) => (
  post.author?._id==profileData?._id && <Post key={post._id || index} post={post} />   // ✅ added key
))}

{postType=="saved" && postData.map((post,index) => (
  userData.saved.includes(post._id) && <Post key={post._id || index} post={post} />   // ✅ added key
))}
