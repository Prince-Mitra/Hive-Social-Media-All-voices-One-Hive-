...
}, [dispatch, selectedUser._id]); // ✅ dependencies

useEffect(() => {
  getAllMessages();
}, [getAllMessages]); // ✅ include the stable reference

useEffect(() => {
  if (!socket) return;

  const handleNewMessage = (mess) => {
    dispatch(setMessages(prev => [...prev, mess])); // functional update avoids stale messages
  };

  socket.on("newMessage", handleNewMessage);

  return () => socket.off("newMessage", handleNewMessage);
}, [socket, dispatch]); // ✅ include socket & dispatch
